...
if __name__ == "__main__":
    ...
    try:
        obj= test_object2(10)
        obj["dut_s11"]["real"] = obj["dut_s11"]["real"][2:]
        clean_twoport(obj)
    except ValueError:
        pass #expected
    else:
    ...
    try:
        ...
        obj[cal_freq][4] = float("nan")
        obj[cal_dut_s11][cal_real][7] = float("nan")
    ...
    assert np.array_equal(obj[cal_freq], ideal[0].f)
    assert np.array_equal(obj[cal_freq], meas[2].f)
    assert np.array_equal(clean_obj[cal_short_s11],np.squeeze((meas[0].s)[:,0,0]))
    assert np.array_equal(clean_obj[cal_open_s11],np.squeeze((meas[1].s)[:,0,0]))
    assert np.array_equal(clean_obj[cal_load_s11],np.squeeze((meas[2].s)[:,0,0]))
    assert np.array_equal(np.zeros(N), np.squeeze(ideal[0].s_db))
    assert np.array_equal(np.zeros(N), np.squeeze(ideal[1].s_db))
    assert np.all(np.less_equal(np.squeeze(ideal[2].s_db), np.ones(N) * -1000))
    assert np.array_equal(np.ones(N) * 180, np.squeeze(ideal[0].s_deg))
